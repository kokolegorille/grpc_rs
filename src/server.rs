...
//     async fn server_stream(&self, request: Request<StreamRequest>) -> Result<ResponseStream> {
//         println!("Streaming...!");
//         println!("Client connected from {:?}", request.remote_addr());

//         let repeat = std::iter::repeat(elt: StreamResponse{
//             message: "Hello from server".to_string();
//             version: "1.0".to_string();
//         });

//         let mut stream = Box::pin(tokio_stream::iter(repeat)
//             .throttle(Duration::from_millis(millis: 500)));

//         let (tx, rx) = mpsc::channel(buffer: 128);

//         tokio::spawn(future: async move{
//             while let Some(item) = stream.next().await {
//                 match tx.send(Result::<_, Status>::Ok(item)).await {
//                     Ok(_) => {
//                         println!("Server Response was queued");
//                     }
//                     Err(_) => {
//                         break;
//                     }
